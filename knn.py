...
def modeling_percentil(function, percentile):
	...
	parameters = [{'n_neighbors': [5, 10, 50, 100], 'weights':['uniform', 'distance'], 'algorithm': ['brute', 'kd_tree']}]
...
def modeling_TruncatedSVD(n_components):
	...
	clf = KNeighborsClassifier()
	parameters = [{'n_neighbors': [5, 10, 50, 100], 'weights':['distance'], 'algorithm': ['brute', 'kd_tree']}]
	grid = GridSearchCV(clf, parameters, cv=5, verbose=10)
	grid.fit(data_train, target_train)
	print(grid.best_params_)
	print(grid.best_score_)
